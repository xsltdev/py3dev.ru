...
Структуры данных -- это, по сути, и есть _структуры_, которые могут хранить
некоторые _данные_ вместе. Другими словами, они используются для хранения
связанных данных.
...
Список[^1] -- это структура данных, которая содержит упорядоченный набор
элементов, т. е. хранит _последовательность_ элементов. Это легко представить,
...
список -- это _изменяемый_ тип данных, т. е. его можно модифицировать.
...
Мы изучим эту тему детально в [её собственной главе](object_oriented_programming.md).
...
однако в список можно добавлять _любые объекты_, включая числа или даже
другие списки.
...
Именно это имеется в виду, когда мы говорим, что списки _изменяемы_, а
строки -- _неизменяемы_.
...
списков. Это называется оператором _индексирования_. Доступ к третьему
...
Мы создаём словарь `ab`[^2] при помощи обозначений, описанных ранее.
...
элементов последовательности. Это ещё называют _приписыванием индекса_.
...
внимание, что полученная вырезка будет _начинаться_ с указанной начальной
позиции, а _заканчиваться_ прямо перед указанной конечной позицией, т.е.
...
позиции 3, и поэтому возвращает _вырезку_ из двух элементов. Аналогично,
...
обозначать _шаг_ вырезки (по умолчанию шаг вырезки равен 1):
...
Множества -- это _неупорядоченные_ наборы простых объектов. Они необходимы
...
_ссылается_ на объект, а не представляет собой этот объект! То есть имя
...
_объекта_, как целое число), вам следует воспользоваться операцией
вырезки. Если вы просто присвоите имя переменной другому имени, оба они
будут _ссылаться_ на один и тот же объект, а это может привести к проблемам,
...
имеется отличный метод для объединения (`join`)[^3] элементов
...
[^2]: **a**ddress **b**ook -- _англ._ "адресная книга" (_прим. перев._)
