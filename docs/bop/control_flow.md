...
Оператор `if` используется для проверки условий: _если_[^1] условие верно[^2], выполняется блок выражений (называемый "if-блок"), _иначе_[^3]
...
программы. Мы узнаем о них больше в [следующей главе](functions.md).
...
так далее -- это называется вложенным[^4] оператором `if`.
...
!!!admonition "Замечание для программистов на C/C++"
    ...
    даже использовать словарь, чтобы сделать это быстро).

## Оператор while

Оператор `while` позволяет многократно выполнять блок команд до тех пор,
пока выполняется некоторое условие. Это один из так называемых _операторов
цикла_. Он также может иметь необязательный пункт `else`.
...
а затем происходит переход к соответствующему _while-блоку_. После выполнения
...
_итерацию_ по последовательности объектов, т.е. проходит через каждый элемент
в последовательности. Мы узнаем больше о последовательностях
...
В этой программе мы выводим на экран _последовательность_ чисел. Мы
генерируем эту последовательность, используя встроенную функцию `range`[^5].
...
только _до_ второго числа, т.е. **не** включает его в себя.
...
`list(range())`. Списки[^6] подробно рассматриваются в главе
Структуры данных.

Затем цикл `for` осуществляет итерацию по этому диапазону - `for i in range(1,5)` эквивалентно `for i in [1, 2, 3, 4]`, что напоминает
...
Оператор `break` служит для _прерывания_[^7] цикла, т. е. остановки
...
_break_, соответствующие им блоки `else` выполняться **не** будут.
...
строкой `'выход'`. Мы останавливаем программу _прерыванием_ цикла
...
пропустить все оставшиеся команды в текущем блоке цикла и _продолжить_[^8]
...
[^8]: continue -- _англ._ "продолжать" (_прим.перев._)
