...
части работают вместе, проектируя и составляя программу, которая _делает_ что-то
полезное. Цель состоит в том, чтобы научиться писать сценарии на языке Python
самостоятельно.

## Задача

Перед нами стоит следующая задача: _Составить программу, которая создаёт
резервные копии всех наших важных файлов_.
...
выберем, _какие_ файлы необходимо копировать? _Как_ их хранить? _Где_ их
хранить?
...
список того, как _я_ себе представляю её работу. Когда вы проектируете
...
5. Будем использовать стандартную команду `zip`, имеющуюся по умолчанию в
   любом стандартном дистрибутиве GNU/Linux.
...
заняться её **отладкой** (дебагом)[^1], т. е. устранением **багов** (ошибок) в
программе.
...
Вы заметили, как мы превратили наш _проект_ в _код_ шаг за шагом.
...
`source`[^2]. Каталог назначения — это каталог, в котором мы сохраняем
...
в GNU/Linux и Unix[^3], `'\\'` в Windows и `':'` в Mac OS.
...
формата можно найти в [справочнике по Python](http://docs.python.org/py3k/library/time.html#time.strftime).

Имя конечного zip-файла мы создаём при помощи оператора, который
_соединяет_ строки, т. е. объединяет две строки и возвращает новую. После
...
"`-q`" используется для указания, что команда должна сработать тихо[^4].
Параметр "`-r`" обозначает, что команда архивации должна работать
рекурсивно[^5] для каталогов, т. е. должна включать все подкаталоги и
...
Затем мы, наконец, _выполняем_ команду при помощи функции `os.system`,
которая запускает команду так, как будто она была запущена из _системы_,
...
!!!admonition "Замечание для пользователей Windows"
    ...
    использоваться "сырые"[^6] строки. Например, можно писать
...
Unix рекомендуется сделать этот программный файл исполнимым,
...
файлов: использование _времени_ в качестве имени файла, сохраняющегося в каталог
...
_Эта программа не работает!_ Python сообщает об обнаружении ошибки
...
указывает нам на место ошибки. Так что мы начинаем _отладку_ программы с
этой строки.
...
добавить уровень _подробности_[^8] вывода, чтобы при указании параметра
...
[zipfile](http://docs.python.org/py3k/library/zipfile.html) вместо вызова
`os.system`.
...
[^1]:
    ...
    [отладку](https://ru.wikipedia.org/wiki/Отладка_программы) (обнаружение
    ...
    например, в записях Томаса Эдисона 1878 года. (_прим. перев._)
...
[^3]:
    ...
    семейство ОС \*BSD, Android, Solaris и т. д. (_прим.перев._)

[^4]: **q**uietly — _англ._ "тихо" (_прим.перев._)
[^5]: **r**ecursive — _англ._ "рекурсивно" (_прим.перев._)
[^6]: raw — _англ._ "сырой", "необработанный" (_прим.перев_)
[^7]: bug fixing — устранение "багов", исправление ошибок (_прим.перев_)
[^8]:
    verbosity — _англ._ "многословность". Применительно к компьютерным
