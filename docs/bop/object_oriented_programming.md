...
манипулируют данными. Это называется _процедурно-ориентированным_ стилем
...
_объектно-ориентированной_ парадигмой программирования. В большинстве случаев
...
объекты. **Класс** создаёт новый _тип_, а **объекты** являются
_экземплярами_ класса. Аналогично, когда мы говорим о "переменных типа `int`",
...
!!!admonition "Замечание для программистов на статических языках"
    ...
    *упаковки и распаковки*[^1].

Объекты могут хранить данные в обычных переменных, которые _принадлежат_
...
_принадлежащие_ классу. Такие функции принято называть **методами** класса.
...
традиции она называется `self`[^2].

Хотя этому параметру можно дать любое имя, _настоятельно рекомендуется_
...
метод полезен для осуществления разного рода _инициализации_, необходимой для
...
как обычными переменными, _заключёнными_ в **пространствах имён** классов и
объектов. Это означает, что их имена действительны только в контексте этих
классов или объектов. Отсюда и название "_пространство имён_".

Существует два типа _полей_: переменные класса и переменные объекта, которые
различаются в зависимости от того, _принадлежит_ ли переменная классу или
объекту соответственно.

_Переменные класса_ разделяемы — доступ к ним могут получать все экземпляры
...
_Переменные объекта_ принадлежат каждому отдельному экземпляру класса. В этом
...
[декораторы](http://www.ibm.com/developerworks/linux/library/l-cpdecor.html):
...
**только** `self`. Это называется _доступом к атрибутам_.
...
а поэтому заранее неизвестно, _когда_ именно этот момент наступит. Чтобы
...
способов этого достичь — при помощи механизма _наследования_. Легче всего
представить себе наследование в виде отношения между классами как _тип и
подтип_.
...
классы преподавателя и студента _наследовали_ этот класс, т.е. чтобы они стали
...
Заметьте также, что код родительского класса _используется многократно_, и нет
необходимости копировать его во все классы, как пришлось бы в случае
использования независимых классов.

Класс `SchoolMember` в этой ситуации называют _базовым классом_ или
_надклассом_[^3]. Классы `Teacher` и `Student` называют _производными
классами_ или _подклассами_[^4].
...
Напротив, если мы _не_ определим метод `__init__` в подклассе, Python
...
Python _всегда_ начинает поиск методов в самом классе, что он и делает в
...
класса, это называется _множественным наследованием_.
...
метаклассы[^5] для создания классов. Метаклассы существуют для изменения или
...
"абстрактные базовые классы". Это означает, что такой класс _абстрактен_, т. е.
...
[^5]:
    в оригинальной версии книги этот параграф невидим для читателей, так как
    ...
    этой концепции здесь." (_прим.перев._)
